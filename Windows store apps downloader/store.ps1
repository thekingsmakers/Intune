...
foreach ($Filename in $MergedList) {
    ...
    $DirectoryPath = Join-Path -Path $BaseDirectory -ChildPath $FolderName
    ...
    try {
        ...
        Write-Warning "Failed to create directory $DirectoryPath : $_"
        continue
    }
    ...
    try {
        ...
        Write-Warning "Failed to download or save $Filename : $_"
        continue
    }
}
