...
function Get-InstallerSilentParams {
    ...
    if ($uninstallLower -match "msiexec\.exe") {
        # Deeper MSI handling: prefer using product code if present, ensure quiet flags, optionally add logging
        ...
        if ($guidMatch.Success) {
            ...
            # Use product code removal which is reliable
            $msiArgs = "/x $guid"
        } else {
            # Strip leading msiexec.exe and capture provided args
            $msiArgs = ($UninstallString -replace "(?i)^\s*msiexec\.exe\s*", "").Trim()
            # Convert passive install switches to full quiet when possible
            $msiArgs = $msiArgs -replace "(?i)/I\b", "/x"
        }

        # Ensure quiet and no restart
        if ($msiArgs -notmatch "(?i)\/qn|\/quiet|\/passive") { $msiArgs = "/qn " + $msiArgs }
        if ($msiArgs -notmatch "(?i)\/norestart") { $msiArgs = $msiArgs + " /norestart" }

        # Optionally add MSI logging to our log dir when running in debug verbosity
        try {
            if ($Verbosity -eq 'debug' -and (Test-Path $LogDir)) {
                $msiLog = Join-Path $LogDir ("msi-uninstall-$(Get-Date -Format 'yyyyMMdd-HHmmss').log")
                # Add logging switch (ensure quoted path)
                if ($msiArgs -notmatch "(?i)/l") { $msiArgs = $msiArgs + " /l*v `"$msiLog`"" }
            }
        } catch { }

        $silentCmd = "msiexec.exe " + ($msiArgs.Trim())
    ...
    # Common unins000.exe pattern (many Inno/other installers)
    if ($uninstallLower -match "unins000\.exe") {
        if ($uninstallLower -notmatch "(?i)/s|/silent|/verysilent|/SILENT|/S") {
            $silentCmd = $UninstallString + " /SILENT"
            Write-Log "unins000.exe detected, adding /SILENT: $silentCmd" 'debug' | Out-Null
            return [string]$silentCmd
        }
        return $UninstallString
    }

    # Update.exe (Squirrel/GitHub Desktop/etc.) - ensure uninstall flag and silent switch
    if ($uninstallLower -match "update\.exe" -or $uninstallLower -match "--uninstall") {
        $silentCmd = $UninstallString
        if ($uninstallLower -notmatch "--uninstall") { $silentCmd += " --uninstall" }
        if ($uninstallLower -notmatch "(?i)(-s|--silent|/s|/quiet)") { $silentCmd += " -s" }
        Write-Log "Update.exe/Squirrel detected, normalized cmd: $silentCmd" 'debug' | Out-Null
        return [string]$silentCmd
    }

    # Firefox / other common setup.exe patterns
    if ($uninstallLower -match "firefox|mozilla" -and $uninstallLower -match "uninstall") {
        if ($uninstallLower -notmatch "(?i)/s|/silent|/verysilent|/SILENT") {
            $silentCmd = $UninstallString + " /S"
            Write-Log "Firefox/Mozilla detected, adding /S: $silentCmd" 'debug' | Out-Null
            return [string]$silentCmd
        }
        return $UninstallString
    }
