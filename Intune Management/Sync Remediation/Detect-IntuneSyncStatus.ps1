...
try {
    ...
    if (-not $enrollmentID) {
        Write-Output "Device is not enrolled in Intune."
        exit 1
    }
    ...
    foreach ($task in $syncTasks) {
        ...
        if ($taskInfo.LastRunTime -and $taskInfo.LastRunTime -gt $lastSyncTime) {
            $lastSyncTime = $taskInfo.LastRunTime
        }
    }

    if (-not $lastSyncTime) {
        $regSyncKey = "HKLM:\SOFTWARE\Microsoft\Enrollments\$enrollmentID\FirstSync"
        $regSync = Get-ItemProperty -Path $regSyncKey -ErrorAction SilentlyContinue
        if ($regSync.SyncTime) {
            $lastSyncTime = [DateTime]::FromFileTimeUtc($regSync.SyncTime).ToLocalTime()
        }
    }

    if (-not $lastSyncTime) {
        Write-Output "Unable to determine last sync time."
        exit 1
    }

    $timeSinceSync = (Get-Date) - $lastSyncTime
    $hoursSinceSync = [math]::Round($timeSinceSync.TotalHours, 2)

    if ($timeSinceSync.TotalHours -ge $thresholdHours) {
        Write-Output "Last Intune sync was $hoursSinceSync hours ago. Remediation required."
        exit 1
    }
    else {
        Write-Output "Last Intune sync was $hoursSinceSync hours ago. Device is compliant."
        exit 0
    }
}
catch {
    Write-Output "Error checking sync status: $($_.Exception.Message)"
    exit 1
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUaTg/VCs4ZBOptpGbsyRqXZPw
# ys+ggg4UMIIGLzCCBBegAwIBAgITTgAAAAcCvNZR4FPQtwAAAAAABzANBgkqhkiG
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFHf6zuSYNtzJKYuP
# 4L/ABSG/AHTJMA0GCSqGSIb3DQEBAQUABIIBAMiUM6T0iNCjsIVXQEkUabsQLvA3
# E0iHNcPY4eaU1gKcu6w2IT2jRr/5OIEz69sHo0WXPV3aD9takOvUyZS57pX/iOaB
# jtAeexLQjc0cK4M0/Dw3wfwKOn3lqlJNsUuA7GVU2tJR4sr82b8/um9mRWi8T43C
# PMWiMPuuKhdHzBs2iHshaFDMP138Rw1RVTSEx4/+jvWaDek20rhSK9QZkKiTeuST
# p+Ru871/L7fzYTxopG+9n9Jb1amHtYcJCCJ4/YcgbC/AKIEu1U4jx3gCn4CqtrmF
# wE3Cc6JlCky317gZdZCFE1qbSmeid0D0iGhFoWjD2TCOGNBcmJYkN97T4x8=
